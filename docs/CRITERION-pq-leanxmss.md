...
**Every signature checked.** The public input binds the signer count and the key set, so the statement cannot shrink. To ensure each signature is actually verified, every submission first runs three negative vectors at the first, the last and a random index; a submission that clears the threshold then runs the full sweep of 900 single-bad-signature inputs, one per index, in parallel. Each run must fail. A guest that skips any index fails the sweep at that index.

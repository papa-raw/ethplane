...
Worknode names are the record. Each of the 65 roadmap nodes is a name under ethplane.eth, and its status, criterion and head live in text records on its own resolver. The verifier holds the writer role for status and head, and the name's owner can write them too; a lineage key that tries gets a revert, on chain, in the rehearsal log. Lineage and guest names are labels rather than enforced identities: the contract records the key that signs and does not check that a key owns the name it registers. Names resolve through the hackathon Universal Resolver on the v2 registry. We built an ENSv2 subregistry and per-node resolvers because the hackathon deployment's implementations cannot be seeded with roles; ours use the contracts-v2 IRegistry interface, permissionless one-time labels and expiry. `ethplane join` rebuilds a worknode's verified state from its name, and docs/ENS-PROBES.md carries every probe with its output.

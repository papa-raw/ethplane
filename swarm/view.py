...
def render():
    ...
    out.append(f"  {D}criterion{X}  cycles strictly below {B}{BASELINE:,}{X} at leanVM a210ef1b, measured by the verifier")
    ...
    out.append(f"  {B}SESSIONS{X} {D}({len(leases)} active · a session says: working on this node, from head H){X}")
    out.append(D + row(["worker", "operator", "since", "heartbeat", "from"], [14, 12, 6, 12, 10]) + X)
    for l in leases[:6]:
        fh = l.get("from_hash", ""); frm = "head" if not fh or set(fh[2:]) == {"0"} else short(fh)
        out.append(row([who(l.get("lineage")), short(l.get("operator")), hhmm(l.get("start")), ago(l.get("last_heartbeat")), frm], [14, 12, 6, 12, 10]))
    ...
    out.append(f"  {B}SUBMISSIONS{X} {D}({len(subs)} · judged by the verifier, recorded on Sepolia){X}")
    out.append(D + row(["artifact", "by", "cycles", "verdict", "reason"], [12, 10, 11, 8, 26]) + X)
    for s in sorted(subs, key=lambda s: s.get("ts") or s.get("block") or 0, reverse=True)[:6]:
        ...
        out.append(row([short(h), who(s.get("lineage")), num(cyc) if cyc else "", "", v.get("reason") or ""], [12, 10, 11, 8, 26]).replace("  " + " " * 8 + "  " + (v.get("reason") or "")[:26].ljust(26), f"  {col}{str(status)[:8].ljust(8)}{X}  " + (v.get("reason") or "")[:26].ljust(26)))
    ...
    out.append(f"  {D}join {X}{AC}{JOIN}{X}{D}  ·  anyone may start a session on any node  ·  refreshed {stamp}{'  ·  api: ' + err if err else ''}{X}")
    return "\n".join(out)
